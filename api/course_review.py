...
from FastApiCourse.db.models import CourseReview
from FastApiCourse.db.schema import  CourseReviewSchema
...
async def create_course_review(review: CourseReviewSchema, db: Session = Depends(get_db)):
...
async def certificate_delete(review_id: int, db: Session = Depends(get_db)):
