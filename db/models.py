from FastApiCourse.db.database import Base
from sqlalchemy.orm import Mapped, mapped_column, relationship
from sqlalchemy import (
    String, Integer, INTEGER, DateTime, Enum, ForeignKey,
...
class UserProfile(Base):
    ...
    user_network: Mapped[List['Network']] = relationship('Network', back_populates='user',
                                                         cascade='all, delete-orphan')
    course_user_teacher: Mapped[List['Course']] = relationship('Course', back_populates='author',
                                                               cascade='all, delete-orphan')
    student_assignment: Mapped[List['Assignment']] = relationship('Assignment',
                                                                  back_populates='students', cascade='all, delete-orphan')
    certificates_students: Mapped[List['Certificates']] = relationship('Certificates',
                                                                       back_populates='students', cascade='all, delete-orphan')
    course_review_user: Mapped[List['CourseReview']] = relationship('CourseReview',
                                                                    back_populates='user', cascade='all, delete-orphan')
    teacher_user_rating: Mapped[List['TeacherRating']] = relationship('TeacherRating',
                                                                      back_populates='teacher', cascade='all, delete-orphan')
    history_students: Mapped[List['History']] = relationship('History',
                                                             back_populates='students', cascade='all, delete-orphan')
    cart_user: Mapped[List['Cart']] = relationship('Cart', back_populates='user',
                                                   cascade='all, delete-orphan')
    user_fav: Mapped[List['Favorite']] = relationship('Favorite', back_populates='user',
                                                          cascade='all, delete-orphan')
    user_token: Mapped[List["Refresh"]] = relationship("Refresh", back_populates='user',
                                                       cascade='all, delete-orphan')
...
class Course(Base):
    ...
    lesson_course: Mapped[List['Lesson']] = relationship('Lesson', back_populates='course',
                                                         cascade='all, delete-orphan')
    course_exam: Mapped[List['Exam']] = relationship('Exam', back_populates='course',
                                                     cascade='all, delete-orphan')
    certificates_course: Mapped[List['Certificates']] = relationship('Certificates',
                                                                     back_populates='course', cascade='all, delete-orphan')
    course_review_course: Mapped[List['CourseReview']] = relationship('CourseReview',
                                                                      back_populates='course', cascade='all, delete-orphan')
    history_course: Mapped[List['History']] = relationship('History', back_populates='course',
                                                           cascade='all, delete-orphan')
    course_fav_item: Mapped[List['FavoriteItem']] = relationship('FavoriteItem', back_populates='course',
                                                           cascade='all, delete-orphan')
...
class FavoriteItem(Base):
    ...
    course_id: Mapped[int] = mapped_column(ForeignKey('course.id'))
    course: Mapped[Course] = relationship(Course, back_populates= 'course_fav_item')
    quantity: Mapped[int] = mapped_column(Integer, default=1)